...
if CommandLine.arguments.count < 2 {
    fputs("usage: swift vision_ocr_image_zh_candidates.swift image.png [image2.png ...]\n", stderr)
    exit(2)
}
...
for imagePath in CommandLine.arguments.dropFirst() {
    ...
    for observation in observations.sorted(by: {
        ...
        let candidates = observation.topCandidates(5).map { $0.string }.joined(separator: "||")
        let box = observation.boundingBox
        print(String(format: "%@\t%.6f\t%.6f\t%.6f\t%.6f\t%@",
                     imageURL.path, box.minX, box.minY, box.width, box.height, candidates))
    }
}
