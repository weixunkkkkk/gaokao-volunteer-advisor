#!/usr/bin/env python3
"""Import GDUFS official major-level admission scores for Guangdong."""
...
BASE_URL = "https://zsxc.gdufs.edu.cn/zsxx/"
PAGE_URL = "https://zsxc.gdufs.edu.cn/zsxx/zsw/lnfs.html"
API_URL = "https://zsxc.gdufs.edu.cn/zsxx/f/ajax_lnfs"
SOURCE_NAME = "广东外语外贸大学本科招生网"
SCHOOL_NAME = "广东外语外贸大学"
PLAN_TYPES = ["普通类", "合作办学", "地方专项计划"]
...
class GdufsClient:
    ...
    def url(self, path: str) -> str:
        return BASE_URL + path + "?ts=" + str(int(time.time() * 1000))
    ...
    def request_json(self, path: str, data: dict[str, object], token: str | None = None) -> tuple[dict[str, object], str]:
        ...
        response = self.opener.open(Request(self.url(path), data=body, headers=headers, method="POST"), timeout=30)
        payload = json.loads(response.read().decode("utf-8", "replace"))
        return payload, response.headers.get("Csrf-Token") or ""

    def ensure_tokens(self, count: int = 5) -> None:
        if len(self.tokens) >= count:
            return
        payload, _ = self.request_json("f/ajax_get_csrfToken", {"n": count})
        if payload.get("state") != 1:
            raise RuntimeError(f"failed to get GDUFS CSRF tokens: {payload}")
    ...
    def post(self, path: str, data: dict[str, object]) -> dict[str, object]:
        ...
        for _ in range(4):
            try:
                ...
                payload, next_token = self.request_json(path, data, token)
                if next_token:
                ...
                self.tokens.clear()
                time.sleep(1.2)
        raise RuntimeError(f"GDUFS request failed after retries: {last_error}")
...
def rank_text(row: dict[str, object]) -> str:
    return score_text(row.get("minRank") or row.get("minOrder"))
    ...
    for query_plan_type in args.plan_types:
        ...
        for item in ((payload.get("data") or {}).get("sszygradeList") or []):
            ...
            if norm(item.get("ssmc")) != args.province or norm(item.get("klmc")) != args.track:
                continue
            row = row_to_admission(
                item,
                batch=args.batch,
                school_code=args.school_code,
                query_plan_type=query_plan_type,
            )
            if row:
                rows.append(row)
...
def parse_years(raw: str) -> list[int]:
    ...
    if unsupported:
        raise argparse.ArgumentTypeError(f"GDUFS importer maps 2023/2024/2025 only, got {unsupported}")
    return years


def parse_plan_types(raw: str) -> list[str]:
    plan_types = [chunk.strip() for chunk in raw.replace("，", ",").split(",") if chunk.strip()]
    unsupported = sorted(set(plan_types) - set(PLAN_TYPES))
    if unsupported:
        raise argparse.ArgumentTypeError(f"unsupported GDUFS plan types: {unsupported}")
    return plan_types


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import GDUFS official Guangdong major-level admission scores.")
    ...
    parser.add_argument("--school-code", default="11846")
